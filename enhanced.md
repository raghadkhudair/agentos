...
| G7 | `ReviewerAgentActor` and `SafetyReviewerAgentActor` issue isolated structured calls per criterion with criterion text, task acceptance, exact artifact checksum/revision, and diff. Provider/tool failure is persisted as `INCONCLUSIVE`; independent identities and self-review rejection remain mandatory. |
| G8, G17 | Schema v4 and `DoDRepository.add_evidence()` bind append-only evidence to project, active criterion version/hash, task, artifact, producer role, subject/integration commit, command/sandbox digests, watched paths/contracts, and evidence generation. Repository checks and `agentos_validate_dod_evidence` enforce the same authority fail closed. |
| G9 | The existing merge path validates task-branch evidence, prepares a governed no-fast-forward prospective tree, runs eligible canonical commands there, commits only on success, records integrated-HEAD command/integration evidence, and conservatively invalidates older watched-path/contract evidence. Durable `integration_attempts` recover crashes before/after the Git and database commits. |
...
| G11 | `DoDEvaluatorActor._evaluate_criterion()` enforces criterion-, task-, and artifact-scoped cardinality exactly as declared, including every mapped task/artifact, command identity, MinIO integrity, reviewer authority, integration ancestry, and revision freshness. |
...
| G13 | `TaskRepository.create_replan_batch()` accepts only typed `InitialTask` graphs covering the durable evaluation's exact gaps. It validates current roles/contracts, artifact/contract coverage, reviewers/security, dependencies/cycles, and one immutable deterministic batch per evaluation generation; exact redelivery is a no-op and a changed duplicate fails closed. |
| G14 | `DoDWatchdog` distinguishes executing, dependency-runnable, and blocked/unclaimable work, uses durable evaluation-correlated event IDs, exponential backoff, bounded attempts, and a visible terminal blocker on exhaustion. |
...
| G20 | `test_dod_contract.py`, the golden dataset, and PostgreSQL/Compose integration tests cover contract rejection, fail-closed bootstrap, isolated criterion verdicts, cache behavior, atomic bootstrap rollback, evidence authority, append-only rows, cardinality/freshness classifications, exact-gap idempotent replanning, restart recovery, evaluation/finalization races, terminal barriers, restricted sandboxing, and the complete integrated delivery path. |
...
- `python -m pytest -q`: 52 passed; 7 integration tests skipped by their explicit environment gate.
- PostgreSQL-focused race/replanning rerun: 2 passed.
- Full isolated Compose topology: 7 integration tests passed, 52 non-integration tests deselected. This includes PostgreSQL, DragonflyDB, MongoDB, MinIO, Milvus, the restricted Docker sandbox, atomic plan/evidence/evaluation controls, exact-gap replanning, and a live `write -> Git/MinIO artifact -> sandbox command -> independent review identity -> prospective merge -> integrated-HEAD evaluator -> atomic DOD_SATISFIED` proof.
